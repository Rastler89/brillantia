...
export default function EditMoldPage() {
  ...
  const { molds, updateMold } = useMolds()
  ...
  const techniqueLabels: Record<MoldTechnique, string> = {
    ...
    estampacion: 'Estampación  "Microfusión',
    electroforming: "Electroforming",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      // Actualizar el molde
      updateMold(params.id as string, updatedMold)

      setSuccess(true)

      // Redirigir después de 1.5 segundos
      setTimeout(() => {
        router.push(`/dashboard/molds/${params.id}`)
      }, 1500)
    } catch (err) {
      console.error("Error al actualizar molde:", err)
      setError("Ocurrió un error al actualizar el molde. Por favor, inténtelo de nuevo.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/dashboard/molds/${params.id}`}>
          <Button variant="outline" size="icon">
            <ArrowLeft className="h-4 w-4" />
          </Button>
        </Link>
        <h1 className="text-2xl font-bold">Editar Molde</h1>
      </div>

      <Card className="bg-card border-border">
        <CardHeader>
          <CardTitle>Información del Molde</CardTitle>
          <CardDescription className="text-muted-foreground">Actualice la información del molde</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {success && (
              <Alert className="bg-primary/10 border-primary/20 text-primary">
                ...
                <AlertDescription>Molde actualizado correctamente</AlertDescription>
              </Alert>
            )}
